...
def test_core_form_configuration_includes_meeting_priorities(monkeypatch) -> None:
    ...
    assert {"425", "F-4", "6-K"}.issubset(settings.expanded_forms)
    assert settings.selected_forms() == settings.forms
    assert settings.expanded_forms.issubset(settings.selected_forms(include_expanded=True))
